...
const process = () => {
    try {
        if (mode.value === 'encode') {
            ...
            props.reportEvent('base64', 'encode_success')
        } else {
            output.value = decodeURIComponent(escape(atob(input.value)))
            props.reportEvent('base64', 'decode_success')
        }
    } catch (e) {
        output.value = "错误: " + e.message
        props.reportEvent('base64', 'error', e.message)
    }
}
...
<template>
  <div class="card">
    <div class="section-header">Base64 编码/解码</div>
